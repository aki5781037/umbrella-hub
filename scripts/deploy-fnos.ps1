...
Invoke-Step -Title "Upload archive to FNOS" -Action {
  ...
  for ($offset = 0; $offset -lt $archiveBase64.Length; $offset += $chunkSize) {
    ...
    $currentChunk | ssh.exe $RemoteHost "cat >> $remoteBase64Path"
    if ($LASTEXITCODE -ne 0) {
      throw "Command failed: upload chunk $chunkIndex to $RemoteHost"
    }
  }

  ssh.exe $RemoteHost "base64 -d -i $remoteBase64Path > $remoteArchivePath && gzip -t $remoteArchivePath && ls -lh $remoteArchivePath && rm -f $remoteBase64Path"
  if ($LASTEXITCODE -ne 0) {
