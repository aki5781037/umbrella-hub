...
import { addProjectComment, getProjectById } from '@/lib/data';
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const author = String(formData.get('author') || project.owner);
  const message = String(formData.get('message') || '');
  const updatedProject = addProjectComment(project.id, message, author);

  if (!updatedProject) {
    return NextResponse.json({ error: 'COMMENTS 保存失败。' }, { status: 400 });
  }
